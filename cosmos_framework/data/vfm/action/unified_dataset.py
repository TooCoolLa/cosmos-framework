...
class ActionUnifiedIterableDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._family_name: str | None = None  # set in assign_worker (single-family path)
    ...
    @classmethod
    def resolve_family_for_rank(
        cls,
        datasets: list[dict[str, Any]],
        rank: int,
        world_size: int,
    ) -> tuple[int, str]:
        """Return ``(family_index, family_name)`` for *rank* under Hare-Niemeyer.

        Same allocation as :meth:`assign_worker`, exposed so callers can
        determine the family this rank will serve *before* worker init
        (e.g. :class:`~.dataloaders.InfiniteDataLoader` patching DataLoader
        kwargs per-family at construction time).

        Raises:
            ValueError: If ``world_size < len(datasets)``.
            IndexError: If ``rank`` is outside ``[0, world_size)``.
        """
        if not (0 <= rank < world_size):
            raise IndexError(f"rank {rank} outside [0, {world_size})")
        rank_ranges = cls._compute_rank_ranges(datasets, world_size)
        for i, (start_rank, end_rank) in enumerate(rank_ranges):
            if start_rank <= rank < end_rank:
                return i, datasets[i].get("name", f"family_{i}")
        raise IndexError(f"rank {rank} not in any of {rank_ranges}")
    ...
    def assign_worker(
        ...
        for i, (start_rank, end_rank) in enumerate(rank_ranges):
            if start_rank <= rank < end_rank:
                ...
                # Stash the family name so per-sample stamping in
                # ``_transform_and_stamp`` can attach it as ``_action_family``
                # on every yielded sample.  The outer ``JointDataLoader``
                # overwrites ``output["dataset_name"]`` with the *stream*
                # name (e.g. ``"action_data"``), which would otherwise lose
                # the per-family identity needed by the
                # ``DetailedDataLoadingSpeedMonitor`` per-rank action
                # metric.  ``_action_family`` is a separate (underscore-
                # prefixed) field that the joint loader passes through
                # untouched via ``_update_output_batch``.
                self._family_name = str(entry.get("name", f"family_{i}"))
        ...
        # Per-rank/worker assignment dump so we can grep the log to see which
        # action dataset family each rank serves and how shards distribute.
        # Useful for diagnosing cross-rank memory skew (heavy families like
        # camera_720 / av_480 land on the unlucky ranks).
        family_name = self._datasets[i].get("name", f"family_{i}") if self._datasets else "<unknown>"
        log.info(
            f"ActionUnifiedIterableDataset assign_worker: rank={rank}/{world_size} "
            f"worker={worker_id}/{num_workers} family={family_name!r} "
            f"resolution={self._resolution} family_ranks={num_family_ranks} "
            f"family_worker_id={family_worker_id}/{family_total_workers} "
            f"num_shards={len(my_shards)}",
            rank0_only=False,
        )

    # -- iteration -------------------------------------------------------------

    def _transform_and_stamp(
        self,
        sample: dict[str, Any],
        resolution: str | None,
        family_name: str | None = None,
    ) -> dict[str, Any]:
        """Apply ``self._transform`` and stamp ``_action_family`` on the sample.

        Single chokepoint for both ``__iter__`` (per-rank single-family path)
        and ``_iter_all_datasets_weighted`` (debug all-families path).  The
        outer ``JointDataLoader`` overwrites ``"dataset_name"`` with the
        *stream* name (e.g. ``"action_data"``), so the per-family identity
        needed by
        :class:`~.callbacks.dataloading_monitor.DetailedDataLoadingSpeedMonitor`
        rides on the underscore-prefixed ``_action_family`` field instead;
        ``_update_output_batch`` passes underscore-prefixed keys through
        untouched.
        """
        sample = self._transform(sample, resolution=resolution)
        if family_name is not None:
            sample["_action_family"] = family_name
        return sample
    ...
    def _iter_all_datasets_weighted(self) -> Iterator[dict[str, Any]]:
        ...
        while True:
            ...
            chosen_family_name = str(self._datasets[chosen].get("name", f"family_{chosen}"))
            try:
                sample = next(iterators[chosen])
                ...
                try:
                    ...
                    continue
            yield self._transform_and_stamp(sample, resolution, chosen_family_name)
