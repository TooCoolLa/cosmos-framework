...
class DatasetEntry:
    ...
    to_opencv: np.ndarray | dict[str, np.ndarray] | None = None  # native EE → OpenCV rotation
...
def _build_datasets() -> dict[str, DatasetEntry]:
    ...
    from cosmos_framework.configs.base.experiment.action.midtrain_config.action_datasets_v1p2 import (
    ...
    agibot_unifier = (
        "cosmos_framework.data.vfm.action.urdf_visualizer.unified_action:to_unified_from_embodiment_c_fk_action"
    )

    from cosmos_framework.data.vfm.action.embodiment_c_fk import AGIBOT_GEAR_GRIPPER_TO_OPENCV_BY_WRIST
    ...
    from cosmos_framework.data.vfm.action.robomind_franka_dataset import _ROBOMIND_FRANKA_TO_OPENCV

    _FRANKA_TO_OPENCV = _ROBOMIND_FRANKA_TO_OPENCV[:3, :3]
    ...
    return {
        ...
        "robomind_ur": _lazycfg_to_entry(
            ...
            fps=10,
            action_format=ActionFormat.SINGLE_ARM_10D,
            to_opencv=np.eye(3, dtype=np.float32),
            viewer_overrides=raw_action_override,
        ),
        ...
        def do_load_episode() -> None:
            ...
            try:
                with _get_dataset_lock(cache_key):
                    ...
                    to_opencv = entry.to_opencv if entry.to_opencv is not None else np.eye(3, dtype=np.float32)
