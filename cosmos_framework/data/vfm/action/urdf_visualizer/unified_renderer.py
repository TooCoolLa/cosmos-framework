...
from cosmos_framework.data.vfm.action.embodiment_c_fk import DEFAULT_GRIPPER_LENGTH_M
from cosmos_framework.data.vfm.action.embodiment_c_spec import get_embodiment_c_kind
from cosmos_framework.data.vfm.action.urdf_visualizer.ik_solver import (
...
from cosmos_framework.data.vfm.action.urdf_visualizer.urdf_loader import (
...
class UnifiedRenderer:
    ...
    def _p(self, path: str) -> str:
        """Prepend the instance name_prefix to a scene path."""
        return f"{self._name_prefix}{path}"
    ...
    def __init__(
        ...
        # Per-instance color palette (allows GT and pred to use different colors)
        pal = palette or {}
        self.COLOR_EGO = pal.get("ego", type(self).COLOR_EGO)
        self.COLOR_EGO_TOP = pal.get("ego_top", type(self).COLOR_EGO_TOP)
        self.COLOR_RIGHT = pal.get("right", type(self).COLOR_RIGHT)
        self.COLOR_LEFT = pal.get("left", type(self).COLOR_LEFT)

        # ── Ego ──
        self.ego_frame = server.scene.add_frame(
            self._p("/ego/frame"), axes_length=self.EGO_AXIS_LENGTH, axes_radius=self.EGO_AXIS_RADIUS
        )
        ...
        self.right_frame = server.scene.add_frame(
            self._p("/right/frame"), axes_length=self.HAND_AXIS_LENGTH, axes_radius=self.HAND_AXIS_RADIUS
        )
        ...
        self.left_frame = server.scene.add_frame(
            self._p("/left/frame"), axes_length=self.HAND_AXIS_LENGTH, axes_radius=self.HAND_AXIS_RADIUS
        )
    ...
    def _rebuild_robot_frame_handles(
        ...
        for frame_key in sorted(frames):
            ...
            handles[frame_key] = self.server.scene.add_frame(
                self._p(f"/robot_{arm}_frames/{kind}/{name}"),
            ...
            if self._current_robot != robot_key:
                ...
                for geometry in geometries:
                    handle = self.server.scene.add_mesh_trimesh(
                        self._p(f"/robot_right/{geometry.name}"),
                        mesh=geometry.mesh,
                    )
                    handle.visible = False
