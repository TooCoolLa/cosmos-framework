...
class VisionDataArgs(ArgsBase, _VisionDataBase):
    ...
    def vision_size(self) -> tuple[int, int]:
        ...
        autodetect via cosmos3.vision.read_and_resize_media before reaching
        ...
        match mode:
            ...
            case ModelMode.INVERSE_DYNAMICS | ModelMode.POLICY:
                if self.raw_action_dim is None:
                    raise ValueError(f"'raw_action_dim' is required for model_mode={mode.value!r}")
            case _:
                assert_never(mode)
...
_CHECKPOINTS: dict[str, CheckpointConfig] = {
    "Cosmos3-Nano": CheckpointConfig(
        ...
        s3_uri="s3://nv-00-10206-checkpoint-experiments/cosmos3_vfm/cosmos3_ga_midtraining/cosmos3_ga_16bm8b_v2_midtrain/checkpoints/iter_000006000/",
        hf=CheckpointDirHf(
            repository="nvidia/Cosmos3-Experimental",
            subdirectory="c075a6e6-646d-4d31-a9ba-b14520391a54",
            revision="05058e4415ec63a325eb77963283589a83b02293",
        ),
    ),
    "Cosmos3-Super": CheckpointConfig(
        ...
        s3_uri="s3://nv-00-10206-checkpoint-experiments/cosmos3_vfm/cosmos3_ga_midtraining/cosmos3_ga_64bm32b_v3p1_midtrain/checkpoints/iter_000000900/",
        hf=CheckpointDirHf(
            repository="nvidia/Cosmos3-Experimental",
            subdirectory="246f3907-a66c-44b7-b2d7-0c8cccb89680",
            revision="f0b727a94133e26bcae335e1270613e30b7e794a",
        ),
    ),
}
