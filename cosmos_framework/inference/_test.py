...
def _check_action_golden(sample_outputs: SampleOutputs, sample_dir: Path) -> list[str]:
    ...
    from cosmos_framework.inference.scripts.eval_utils import compute_sample_metrics
...
_SFT_GET_ENV = {
    ...
    "policy": SftGetEnv(
        ...
        job_name="action_policy_sft_8b",
        dataset_name="nvidia/LIBERO_LeRobot_v3",
    ),
    "forward_dynamics": SftGetEnv(
        base_checkpoint_name="Cosmos3-Nano",
        config_file="cosmos3/configs/experiment/action_fdm_sft_nano.yaml",
        job_name="action_fdm_sft_nano",
        dataset_name="nvidia/bridge_lerobot_v3",
    ),
}
