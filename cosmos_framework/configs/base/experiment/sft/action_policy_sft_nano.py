...
"""Action policy SFT on the LIBERO suite — "nano" variant.
...
Usage::
    ...
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 \\
        ...
        experiment=action_policy_sft_nano checkpoint.load_path=<path>
"""
...
def _action_policy_nano_model_config() -> dict:
...
action_policy_sft_nano = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            ...
            group="action_libero",
            name="action_policy_sft_nano",
            wandb_mode="offline",
        ),
        model=dict(
            config=_action_policy_nano_model_config(),
        ),
        optimizer=dict(
            betas=[0.9, 0.99],
            eps=1.0e-08,
...
for _item in [action_policy_sft_nano]:
