...
"""Action forward-dynamics SFT on Bridge LeRobot — "super" variant.
...
Dataset root is taken from ``$DATASET_PATH`` (Bridge LeRobot root, same as the
nano FDM variant). ``checkpoint.load_path`` is a required override (Hydra
``???`` placeholder); supply it on the CLI or in a downstream experiment.
...
Usage::
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 PYTHONPATH=. torchrun --nproc_per_node=8 \\
        ...
        experiment=action_fdm_sft_super checkpoint.load_path=<path>
"""
...
def _action_fdm_super_model_config() -> dict:
    """SUPER_MODEL_CONFIG baseline + action-FDM-specific overrides.

    Mirrors the action_fdm_sft_nano pattern (action_gen on, FDM-style loss
    scales, encode_exact_durations=[17]) on top of the 32B / LoRA / DP=4 /
    CP=2 super baseline.
    """
...
action_fdm_sft_super = LazyDict(
    dict(
        ...
        job=dict(
            ...
            group="action_bridge",
            name="action_fdm_sft_super",
            wandb_mode="offline",
        ),
        model=dict(
            config=_action_fdm_super_model_config(),
        ),
        optimizer=dict(
            betas=[0.9, 0.99],
            eps=1.0e-08,
...
for _item in [action_fdm_sft_super]:
