...
"""``vision_sft_super`` — 1-to-1 conversion of

    cosmos3-internal/cosmos3/configs/experiment/vision_sft_super.yaml

into the prerelease layout. Sibling of ``vision_sft_nano``: same source-YAML
shape, same Hydra defaults, same dataloader stack
(PackingDataLoader + RankPartitionedDataLoader), same three forced
bucket-C deviations (see docs/vision_sft_nano_conversion.md). Key model-side
differences vs nano:
  ...
  * Parallelism: ``data_parallel_shard_degree=4``,
    ``context_parallel_shard_degree=2``, ``use_torch_compile=False``.
...
Translation notes (mirror vision_sft_nano.py):
  * ``cosmos3._src.*`` paths rewritten to ``cosmos_framework.*``.
  * ``_self_`` placed LAST in defaults (prerelease convention) instead
    of FIRST (YAML convention) — see vision_sft_nano.py docstring.
