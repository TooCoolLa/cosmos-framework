...
"""``vision_sft_nano`` — 1-to-1 conversion of

    cosmos3-internal/cosmos3/configs/experiment/vision_sft_nano.yaml

into the prerelease layout. Same Qwen3-VL-8B / Cosmos3-Nano backbone, T2V/I2V/V2V
SFT, PackingDataLoader + RankPartitionedDataLoader stack. EMA enabled.

Translation notes (compared with the source YAML):
    * ``cosmos3._src.*`` module paths were rewritten to ``cosmos_framework.*`` (the
      prerelease module layout).
