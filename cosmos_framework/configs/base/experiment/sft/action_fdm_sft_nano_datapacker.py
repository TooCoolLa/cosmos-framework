...
"""Action FDM SFT on Bridge LeRobot — DataPackerDataLoader variant.

Equivalent to ``action_fdm_sft_nano`` but replaces the
IterativeJointDataLoader + InfiniteDataLoader stack with DataPackerDataLoader
+ BridgeFDMDataPacker.  Sample processing is identical (same
ActionTransformPipeline params); batching changes from fixed batch_size=4 to
token-budget packing capped at max_batch_size=64.

Usage::

    DATASET_PATH=/path/to/bridge_orig_lerobot torchrun \\
        --nproc_per_node=4 --master_port=12341 -m cosmos_framework.scripts.train \\
        --config=configs/base/config.py -- \\
        experiment=action_fdm_sft_nano_datapacker \\
...
from cosmos_framework.configs.base.experiment.sft.action_fdm_sft_nano import action_fdm_sft_nano
...
def _get_bridge_iterable_dataset(**kwargs) -> MapToIterableAdapter:
    """Wrap BridgeOrigLeRobotDataset in MapToIterableAdapter for DataPackerDataLoader.

    BridgeOrigLeRobotDataset uses deferred shard registration: _register_sources()
    must be called before the dataset yields any items. wrap_dataset() does this via
    ActionUnifiedIterableDataset.assign_worker; here we call it explicitly.
    """
    ds = BridgeOrigLeRobotDataset(**kwargs)
    ds._register_sources()
    return MapToIterableAdapter(ds)
...
class BridgeFDMDataPacker(DataPacker):
    """DataPacker adapter for BridgeOrigLeRobotDataset + ActionTransformPipeline (FDM).

    Applies the same ActionTransformPipeline as the wrap_dataset call in
    action_fdm_sft_nano.  The sft_collate_fn output format matches what
    OmniMoTModel.training_step expects (same as ActionDataPacker in the
    libero_policy_datapacker_experiment).
    """
...
_exp = copy.deepcopy(action_fdm_sft_nano)
_exp["dataloader_train"] = L(DataPackerDataLoader)(
    data_source=L(_get_bridge_iterable_dataset)(
    ...
    data_packer=L(BridgeFDMDataPacker)(
...
_exp["job"]["name"] = "action_fdm_sft_nano_datapacker_${now:%Y-%m-%d}_${now:%H-%M-%S}"
...
cs.store(group="experiment", package="_global_", name="action_fdm_sft_nano_datapacker", node=_exp)
