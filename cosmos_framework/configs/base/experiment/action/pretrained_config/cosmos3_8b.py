...
"""Centralized Cosmos3 VFM 8B pretrained model config for Action experiments.

Provides make_8b_experiment(), a factory that builds complete experiment configs
from the 8B multires recipe_v7 pretrained base (originally 720p, fine-tuned at 480p).

Usage — downstream experiment files just pass datasets::

    from cosmos_framework.configs.base.experiment.action.pretrained_config.cosmos3_8b import (
        make_8b_experiment,
    )

    exp = make_8b_experiment("my_exp", [DATASET_BRIDGE, DATASET_UMI])
...
The raw ``cosmos3_pretrained_8b`` dict is also registered in ConfigStore for
Hydra defaults inheritance (``defaults=["/experiment/cosmos3_pretrained_8b", "_self_"]``).
"""
...
PRETRAINED_CHECKPOINT = (
    ...
    "cosmos3_vfm/t2w_mot_8b_qwen3_vl_runs/t2w_mot_exp406_000_qwen3_vl_8b_multires_recipe_midtraining_v5/checkpoints/iter_000035000/"  # SOT52
)
...
def make_8b_experiment(
    ...
    *,
    ...
    """Build a Cosmos3 8B Action experiment config with the given dataset subset.

    Args:
        extra_dataloaders: Additional dataloader entries (e.g. VFM image/video
            dataloaders) to include alongside ``action_data`` in the
            ``IterativeJointDataLoader``.  Keys are dataloader names and values
            are ``dict(dataloader=..., ratio=...)`` entries.
        keys_to_skip_loading: Checkpoint keys to skip when loading.  Defaults to
            action-specific layers.  Pass ``[]`` when resuming from an action
            checkpoint to load all weights.
    """
    ...
    return dict(
        ...
        job=dict(
            ...
            group="cosmos3_action_8b_pretrained",
            name=exp_name,
        ),
        model=dict(
            config=dict(
...
# ---------------------------------------------------------------------------
# Hydra defaults inheritance config generated from make_8b_experiment().
# ---------------------------------------------------------------------------
cosmos3_pretrained_8b = make_8b_experiment("cosmos3_pretrained_8b", datasets=[])
cosmos3_pretrained_8b["job"]["group"] = "pretrained_config"
cosmos3_pretrained_8b["job"]["name"] = "cosmos3_pretrained_8b"

cs = ConfigStore.instance()
cs.store(
    group="experiment",
    package="_global_",
    name="cosmos3_pretrained_8b",
    node=cosmos3_pretrained_8b,
)
