...
Usage (smoke test — same overrides as launch_action_from_vfm.sh)::
    ...
    LIBERO_LOCAL_DATA_ROOT=outputs/libero_datasets torchrun \\
        --nproc_per_node=4 --master_port=12342 -m cosmos_framework.scripts.train \\
...
from cosmos_framework.configs.base.experiment.action.posttrain_config.libero_policy_experiment import (
    _LIBERO_REPO_IDS,
    _LIBERO_ROOTS,
...
# data_source factory
#
# DataPackerDataLoader accepts map-style Datasets directly — with shuffle=True
# it wraps LIBERODataset in _ShuffledMapIterableDataset internally, so no
# manual MapToIterableAdapter wrapping is needed.
# ---------------------------------------------------------------------------


def get_libero_map_dataset(**kwargs) -> LIBERODataset:
    """Return LIBERODataset as a map-style Dataset for DataPackerDataLoader.

    DataPackerDataLoader(shuffle=True) wraps it in _ShuffledMapIterableDataset,
    giving per-epoch randperm shuffle and stateful checkpoint/resume via
    DataLoaderStateCallback(distributor_type="data_packer").
    """
    return LIBERODataset(**kwargs)
...
def action_policy_sft_8b_datapacker_experiments() -> None:
    ...
    # Mirror cosmos-inference action_policy_sft_nano.yaml: replace the
    # "training_stats" callbacks group (set by _build_libero_policy_base_8b)
    # with "generation". every_n_sample_{ema,reg} are pinned below to
    # every_n=999_999 so they never fire during the 16k-iter run, matching the
    # YAML's effectively-disabled stance.
    for _i, _d in enumerate(exp["defaults"]):
        if isinstance(_d, dict) and "override /callbacks" in _d:
            exp["defaults"][_i] = {
                "override /callbacks": [
                    "basic",
                    "optimization",
    ...
    exp["checkpoint"]["save_iter"] = 100
    exp["trainer"]["callbacks"]["compile_tokenizer"]["warmup_resolutions"] = ["256", "480", "720"]
    exp["trainer"]["callbacks"]["every_n_sample_ema"] = dict(every_n=999_999, save_s3=False)
    exp["trainer"]["callbacks"]["every_n_sample_reg"] = dict(every_n=999_999, save_s3=False)
    ...
    # local checkpoint_path / wan_vae_path declared in
    # examples/toml/launch_action_libero.toml and the HF tokenizer/weights
    # stack. Without these, defaults pull the GCP bucket +
    # qwen3_vl_mot_vlm_8b_instruct_gcp variant.
    ...
    # DataLoaderStateCallback tracks per-worker (epoch, position) when shuffle=True
    # is passed to DataPackerDataLoader.  It is a no-op when the batch carries no
    # position metadata (shuffle=False or iterable data_source).
    ...
    # Pass LIBERODataset directly as a map-style Dataset — DataPackerDataLoader
    # wraps it in _ShuffledMapIterableDataset internally (shuffle=True), so
    # MapToIterableAdapter is no longer needed.
    # max_tokens=999_999 ensures the token budget never triggers;
    # max_batch_size=128 mirrors action_policy_sft_nano.yaml's per-step batch
    # (inner InfiniteDataLoader.batch_size=128 under IterativeJointDataLoader).
    exp["dataloader_train"] = L(DataPackerDataLoader)(
        data_source=L(get_libero_map_dataset)(
            repo_id=copy.deepcopy(_LIBERO_REPO_IDS),
            root=copy.deepcopy(_LIBERO_ROOTS),
            fps=20,
            camera_mode="concat_view",
