...
from cosmos_framework.configs.base.experiment.action._experiment_helpers import (
...
from cosmos_framework.configs.base.experiment.action.pretrained_config.cosmos3_8b import make_8b_experiment

ACTION_STATS_PATH = "cosmos_framework/data/vfm/action/libero_action_stats_10k.json"
NATIVE_FRAME_WISE_ROT6D_ACTION_STATS_PATH = (
    "cosmos_framework/data/vfm/action/normalizers/libero_native_frame_wise_relative_rot6d.json"
)
cs = ConfigStore.instance()

_LIBERO_REPO_IDS = ["libero_10", "libero_object", "libero_spatial", "libero_goal"]

# Cluster layout (default). Override per-suite paths to point at your data.
...
def _resolve_libero_roots() -> list[str]:
    ...
    debug machines without the shared cluster mount can opt into a local mount
    via the same env var that the other libero experiments already respect.
    """
...
def multi_view_policy_exp():
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_multi_view_lr2e4
    """
    ...
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_multi_view_lr2e4_20k
    """
...
def single_view_policy_exp():
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_single_view_lr2e4_60k
    """
...
def normalized_action_policy_exp():
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_norm_action_lr2e4_60k
    """
    ...
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_norm_action_wrist_lr2e4_60k
    """
...
def wrist_view_policy_exp():
    """
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=. torchrun --nproc_per_node=1 --master_port=12341 cosmos_framework/scripts/train.py --config=cosmos_framework/configs/base/config.py -- experiment=libero_exp_policy_wrist_view_lr2e4_60k
    """
