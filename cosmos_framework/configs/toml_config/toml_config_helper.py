...
PATH_REMAPS: dict[str, dict[tuple[str, ...], "tuple[str, ...] | None"]] = {
    ...
    # VLM (VLMModelConfig): model.config.{policy, train, freeze, ema} —
    # parallelism / activation_checkpointing nest under ``.policy.*``.
    # Fields that have no VLM analog map to ``None`` (skip).
    "vlm": {
        ...
        ("model", "parallelism"): ("model", "config", "policy", "parallelism"),
        ("model", "activation_checkpointing"): ("model", "config", "policy", "activation_checkpointing"),
