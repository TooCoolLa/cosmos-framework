...
class ParallelismConfig(BaseModel):
    ...
    Lands at ``model.config.parallelism.*`` on VFM and at
    ``model.config.policy.parallelism.*`` on VLM (see PATH_REMAPS).
    """
    ...
    use_torch_compile: bool = Field(
        default=False,
        description=(
            "torch.compile the network. Big speedup on stable shapes; "
            "conflicts with some custom CUDA kernels and deterministic modes."
        ),
    )
    compile_dynamic: bool = Field(
        default=True,
        description=(
            "When use_torch_compile=True, recompile per-shape rather than "
            "specializing for one static shape. Required for the "
            "compile_tokenizer callback's progressive warmup."
        ),
    )
    ...
    precision: str = Field(
        default="bfloat16",
        description=(
            "Compute dtype for the network forward/backward. 'bfloat16' is "
            "standard for Hopper/Blackwell. Master weights live in fp32 "
            "separately (see TrainConfig on VLM)."
        ),
    )
...
class ActivationCheckpointingConfig(BaseModel):
    ...
    Lands at ``model.config.activation_checkpointing.*`` (VFM) or
    ``model.config.policy.activation_checkpointing.*`` (VLM).
    """
...
class ModelConfig(BaseModel):
    ...
    Lands at ``model.config.*`` on VFM and on VLM, with sub-tree paths
    remapped per PATH_REMAPS (parallelism / activation_checkpointing nest
    under ``.policy.*`` on VLM).
    """
