...
_PRECISION_TO_TORCH_DTYPE: dict[str, torch.dtype] = {
...
def parallelize(
    ...
    Args:
        ...
        precision:     Activation / parameter dtype for FSDP MixedPrecisionPolicy
                       (one of ``"bfloat16"``, ``"float16"``, ``"float32"``).
                       Sourced from ``policy.parallelism.precision``.
        fsdp_offload:  If True, wrap with CPUOffloadPolicy. Sourced from
                       ``train.fsdp_offload``.
    """
    ...
    mp_policy = MixedPrecisionPolicy(
        param_dtype=_PRECISION_TO_TORCH_DTYPE[precision],
    ...
    cpu_offload_policy = CPUOffloadPolicy() if fsdp_offload else None
