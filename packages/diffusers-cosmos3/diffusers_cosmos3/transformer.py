...
class Cosmos3VLTextAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.q_norm = Cosmos3VLTextRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
...
class Cosmos3OmniTransformer(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if action_gen:
            self.action2llm = DomainAwareLinear(self.action_dim, hidden_size, self.num_embodiment_domains)
            self.llm2action = DomainAwareLinear(hidden_size, self.action_dim, self.num_embodiment_domains)
