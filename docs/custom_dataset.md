...
### Generator — Video text-to-world SFT

**File**: `cosmos_framework/configs/base/experiment/posttrain_video/t2w_sft_8b_local_datapacker.py`

```
data_source:  SFTDataset (local JSONL + video files, IterableDataset)
DataPacker:   SFTDataPacker
  sft_process_sample:  pass-through (SFTDataset pre-decodes everything)
  compute_num_tokens:  VAE spatial/temporal formula on video shape
  sft_collate_fn:      nested list format for OmniMoTModel
max_batch_size: 1
max_tokens:    45056  (720p video budget)
shuffle:       False  (SFTDataset is an IterableDataset)
```

Override JSONL path from CLI — no Python changes needed:

```bash
torchrun ... experiment=t2w_sft_8b_local_datapacker \
    'dataloader_train.data_source.jsonl_paths=[/path/to/your_videos.jsonl]'
```
