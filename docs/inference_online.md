...
```shell
python -m cosmos_framework.inference.ray.serve --help
```
...
```shell
serve run cosmos_framework/inference/ray/configs/latency.yaml
```
...
```shell
python -m cosmos_framework.inference.ray.submit \
    -i "inputs/omni/*.json" \
    -o outputs/omni_ray \
    --seed=0
```
...
```shell
python -m cosmos_framework.inference.ray.submit --help
```
...
```shell
python -m cosmos_framework.inference.ray.gradio --port=8080
```

To see all available arguments:

```shell
python -m cosmos_framework.inference.ray.gradio --help
```
