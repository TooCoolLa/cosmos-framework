...
| `experiment` | `""`         | **META** — names the Hydra experiment LazyDict registered in `ConfigStore` under `experiment/<name>`. Resolved at load time via `experiment=<name>` (e.g. `action_fdm_sft_nano`).                                                    |
...
FSDP / context-parallel / classifier-free-guidance topology. Lands at `model.config.parallelism.*` on VFM and at `model.config.policy.parallelism.*` on VLM.

| field                            | default      | description                                                                                                                                                                |
| -------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `use_torch_compile`              | `false`      | `torch.compile` the network. Big speedup on stable shapes; conflicts with some custom CUDA kernels and deterministic modes.                                                |
| `compile_dynamic`                | `true`       | When `use_torch_compile=true`, recompile per-shape rather than specializing for a single static shape. Required for the `compile_tokenizer` callback's progressive warmup. |
...
| `precision`                      | `"bfloat16"` | Compute dtype for forward/backward. `"bfloat16"` is standard for Hopper/Blackwell. Master weights live in fp32 separately (see VLM `TrainConfig`).                         |
...
Recompute activations during backward to trade FLOPs for memory. Lands at `model.config.activation_checkpointing.*` (VFM) or `model.config.policy.activation_checkpointing.*` (VLM).
...
| `model.parallelism.*`                                                                    | `model.config.parallelism.*`              | `model.config.policy.parallelism.*`              |
| `model.activation_checkpointing.*`                                                       | `model.config.activation_checkpointing.*` | `model.config.policy.activation_checkpointing.*` |
...
| key                                                              | purpose                                                                                                     | used by                                                            |
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `dataloader_train.dataloaders.action_data.dataloader.batch_size` | Inner action_data sub-loader prefetch batch (per-worker memory knob, **not** the effective training batch). | `launch_sft_action_*_super.sh`                                     |
